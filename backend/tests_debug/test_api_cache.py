...
def test_api():
    ...
    print("1️⃣ GET /api/suggestions (0 entries)")
    ...
    print("2️⃣ POST /api/suggestions/save (entry 1)")
    ...
    print("3️⃣ POST /api/suggestions/save (entry 2)")
    resp = requests.post(f"{BASE_URL}/suggestions/save", params={
    ...
    # Test 4: Get suggestions (still no suggestions - need >=3 entries for suggestions to appear)
    print("4️⃣ GET /api/suggestions (2 entries - should still be no suggestions)")
    resp = requests.get(f"{BASE_URL}/suggestions", params={"user_id": 1, "field_id": 1})
    data = resp.json()
    print(f"  Entry count: {data['entry_count']}")
    print(f"  Is first entry: {data['is_first_entry']}")
    ...
    print("5️⃣ POST /api/suggestions/save (entry 3)")
    resp = requests.post(f"{BASE_URL}/suggestions/save", params={
        "user_id": 1, "field_id": 1, "form_id": 1, "value": "Hanoi"
    })
    entry3 = resp.json()
    ...
    print("6️⃣ GET /api/suggestions (3+ entries - should show suggestions)")
    resp = requests.get(f"{BASE_URL}/suggestions", params={"user_id": 1, "field_id": 1})
    data = resp.json()
    print(f"  Entry count: {data['entry_count']}")
    print(f"  Is first entry: {data['is_first_entry']}")
    ...
    print("7️⃣ Verify suggestions are from cache table (not calculated real-time)")
    db = SessionLocal()
    try:
        ...
        if len(suggestions) > 0:
            print(f"\n  ✅ Suggestions are being read from cache!")
            return True
        else:
            print(f"\n  ❌ Suggestions cache is empty!")
            return False
    finally:
        db.close()
...
if __name__ == "__main__":
    try:
        ...
        print("\n" + "=" * 70)
        if success:
            print("✅ ALL TESTS PASSED - Suggestions cache is working correctly!")
        else:
            print("❌ TEST FAILED")
        print("=" * 70)
